    ...
    private void saveAsCsv(List<ContactData> contacts, File file) throws IOException {
        ...
        for (ContactData contact : contacts) {
            writer.write(String.format("%s;%s;%s;%s;%s;%s\n", contact.getFirstname(),contact.getLastname(),
                    contact.getHomePhone(), contact.getMobilePhone(), contact.getAddress(),contact.getEmail()));
        }
        writer.close();
    }
    ...
    private List<ContactData> generateContacts(int count) {
        ...
        for (int i =0; i < count; i++) {
            contacts.add(new ContactData().withFirstname(String.format("name %s", i))
                    ...
                    .withMobilePhone(String.format("mobilephone %s", i)).withAddress(String.format("address %s", i))
